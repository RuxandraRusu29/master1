...
class JunctionTree:
    ...
    def _triangulate(self, h: nx.Graph) -> nx.Graph:
        #print("chordal graph")
        #print(complete_to_chordal_graph(h)[0].edges)
        return complete_to_chordal_graph(h)[0]

    def _create_clique_graph(self, th: nx.Graph) -> nx.Graph:
        aux = nx.chordal_graph_cliques(th)
        G = nx.Graph()
        G.add_nodes_from(aux)
        G.add_weighted_edges_from([(frozenset({'A', 'B'}), frozenset({'D', 'B', 'C'}),1),
                    (frozenset({'B', 'C', 'D'}), frozenset({'C', 'D', 'E'}),2),
                    (frozenset({'B', 'C', 'D'}), frozenset({'D', 'E', 'G'}),1),
                    (frozenset({'B', 'C', 'D'}), frozenset({'F', 'D'}),1),
                    (frozenset({'F', 'I'}), frozenset({'F', 'D'}),1),
                    (frozenset({'H', 'I', 'K'}), frozenset({'F', 'I'}),1),
                    (frozenset({'F', 'D'}), frozenset({'C', 'D', 'E'}),1),
                    (frozenset({'C', 'D', 'E'}), frozenset({'D', 'E', 'G'}),2),
                    (frozenset({'F', 'D'}), frozenset({'D', 'E', 'G'}),1),
                    (frozenset({'C', 'D', 'E'}), frozenset({'E', 'G', 'J'}),1),
                    (frozenset({'D', 'E', 'G'}), frozenset({'E', 'G', 'J'}),2),
                    (frozenset({'E', 'G', 'J'}), frozenset({'J', 'L'}),1)])
        #print("clique tree")
    ...
    def _load_factors(self) -> None:
        ...
        for node in self.clique_tree.nodes():
            columns = list(node)
            num_parents = len(columns)
            product_list = [[1, 0]] + [[0, 1]] * (num_parents - 1)
            ...
            index = pd.MultiIndex.from_tuples(cpt_idx, names=idx_names)
            cpd_df = pd.DataFrame(data=1.0, index=index, columns=["prob"])
            dict[node] = cpd_df
            print(dict[node])
        
        print("second part")
        for var_name in bn.nodes:
            node = bn.nodes[var_name]
            for tn in self.clique_tree.nodes():
                if node.var_name in list(tn) and all(item in list(tn) for item in [x.var_name for x in node.parent_nodes]):
                    print(dict[tn])
                    dict[tn] = multiply_factors(dict[tn], node.cpd)
                    break
        self.dict = dict
    ...
    def _incorporate_evidence(self, jt: nx.DiGraph, evidence: Dict[str, int]) -> nx.DiGraph:
        ...
        for key in evidence:
            for tn in jt.nodes():
                if key in list(tn) and key in self.dict[tn].index.names:
                    df = self.dict[tn]
                    df.iloc[df.index.get_level_values(key) != int(evidence[key])] = 0
                    self.dict[tn] = df
                    break
            

    def propagate_to_root(self, uncalibrated_jt, node):
        pass
